document.addEventListener("DOMContentLoaded", () => {
  ...
  const roleNavItem = document.querySelector('.nav-item a.nav-link[href="#"]');
  if (roleNavItem) {
...
async function loadTableData() {
  try {
    ...
    const data = await response.json();
    tableData = data; // Store the fetched data for pagination
...
function renderTable() {
  ...
  // Hide buttons if the owner is not the same as the current user
  if (owner.trim() !== userEmail.trim()) {
    const buttonsToHide = [".btn-edit", ".btn-delete"];
    buttonsToHide.forEach((selector) => {
      document.querySelectorAll(selector).forEach((button) => {
...
async function loadParticipants() {
  ...
  const response = await fetch(`/read?jsonStr=${encodeURIComponent(data)}`);
  const result = await response.json();
  console.log("fetch participants:", data);
  console.log(result);
  participantsData = result.userIDs || []; // Store the fetched participants data for pagination
...
function renderParticipants() {
  ...
  // Determine which participants to show for the current page
  const start = (currentPage - 1) * rowsPerPage;
  const end = start + rowsPerPage;
  const participantsToDisplay = participantsData.slice(start, end);

  participantsToDisplay.forEach((userID) => {
  ...
  // Hide delete button if the owner is not the current user
  if (owner.trim() !== userEmail.trim()) {
...
async function loadCategories() {
  ...
  try {
    ...
    if (data.categories && data.categories.length > 0) {
      const categorySelectAdd = document.getElementById("categoryDropdown");
      ...
      if (data.categories && data.categories.length > 0) {
        ...
        data.categories.forEach((category) => {
          ...
          listItem.addEventListener("click", () => {
            ...
            document.getElementById("filterCategoryButton").textContent =
              category;
          });
